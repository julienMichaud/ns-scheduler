	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if !scalingUp {
			...
			if value, ok := result.Annotations["ns-scheduler/originalReplicas"]; ok {
				...
				fmt.Printf("annotation 'ns-scheduler/originalReplicas' not found for deployment %s of namespace %s, will set replicas to 3 by default", result.ObjectMeta.Name, ns)
				i := int32(3)
				result.Spec.Replicas = int32Ptr(i)
			}
		}
